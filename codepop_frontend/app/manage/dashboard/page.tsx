...
export default function StoreManagerDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      try {
        const inventoryRes = await fetch(apiUrl('/backend/inventory/report/'), { headers });
        if (!inventoryRes.ok) {
      ...
      try {
        const revenueRes = await fetch(apiUrl('/backend/revenues/'), { headers });
        if (!revenueRes.ok) {
          setRevenues([]);
      ...
      try {
        const flavorRes = await fetch(apiUrl('/backend/flavors/'), { headers });
        if (!flavorRes.ok) {
          setFlavors([]);
      ...
      try {
        const profileRes = await fetch(apiUrl('/backend/users/me/'), { headers });
        if (!profileRes.ok) {
      ...
      try {
        const ordersRes = await fetch(apiUrl('/backend/orders/'), { headers });
        if (!ordersRes.ok) {
  ...
  const submitFlavorDetails = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const flavorResponse = await fetch(
        selectedFlavorId === ''
          ? apiUrl('/backend/flavors/')
          : apiUrl(`/backend/flavors/${selectedFlavorId}/`),
        {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(apiUrl(`/backend/orders/${orderId}/`), {
        method: 'PATCH',
        headers: {
  ...
  const submitUsageReport = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(apiUrl(`/backend/inventory/${selectedInventoryId}/`), {
        method: 'PATCH',
        headers,
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      await fetch(apiUrl('/backend/auth/logout/'), {
        method: 'POST',
        headers,
        ...
        {dashboardView === 'report' ? (
          <>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              >
                <ClipboardList size={18} /> Report Usage
              </button>
              <button
                onClick={openFlavorModal}
                className="bg-white border-2 border-gray-300 text-gray-800 py-3 rounded-lg font-semibold flex items-center justify-center gap-2 hover:bg-gray-50"
              >
                <SlidersHorizontal size={18} /> Edit Flavor Details
              </button>
            </div>
