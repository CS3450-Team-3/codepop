...
const AIAlert = ({ isModalVisible, toggleModal, drinkDict }) => {
  ...
  const createObj = async () => {
    try {
      ...
      const sodaUsed =
        Array.isArray(drinkDict.SodaUsed) && drinkDict.SodaUsed.length > 0
          ? drinkDict.SodaUsed
          : [drinkDict.SodaUsed];
      const syrupsUsed = Array.isArray(drinkDict.SyrupsUsed)
        ? drinkDict.SyrupsUsed
        : [];
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to create drink: ${response.status} - ${errorText}`
        );
      }
