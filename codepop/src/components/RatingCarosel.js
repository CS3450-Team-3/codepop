...
const RatingCarosel = ({ purchasedDrinks }) => {
  ...
  const addToFavs = async (drink) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to add to favorites. Status: ${response.status}`
        );
      }
    } catch (error) {
    ...
    return (
      <View style={styles.carouselItem}>
        ...
        <Text style={styles.drinkName}>
          {drink.SodaUsed} with {drink.SyrupsUsed?.join(", ")}{" "}
          {drink.AddIns?.join(", ")}
        </Text>
        ...
        <View style={styles.middleContainer}>
          <TouchableOpacity
            onPress={() => console.log("Drink added to favorites")}
            style={styles.button}
          >
        ...
        <StarRating
          ...
          onRatingSelected={(newRating) =>
            handleRatingSelected(newRating, drink)
          }
        />
      </View>
    );
