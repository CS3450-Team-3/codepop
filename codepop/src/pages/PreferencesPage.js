  ...
  const fetchUserPreferences = async (token, userId) => {
    try {
      ...
      const filteredSoda = preferences
        .filter((item) =>
          filterInventory("Soda").some(
            (inventoryItem) =>
              inventoryItem.value.toLowerCase() ===
                item.Preference.toLowerCase() &&
  ...
  const removePreferences = async (pref) => {
    try {
      ...
      const filteredPreferences = preferences.filter(
        (item) =>
          String(item.UserID) === String(userId) &&
          item.Preference.toLowerCase() === pref.toLowerCase()
      );

      for (let preference of filteredPreferences) {
        const deleteResponse = await fetch(
          `${BASE_URL}/backend/preferences/${preference.PreferenceID}/`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
          ...
          <>
            ...
            {isLoggedIn ? (
              <>
                ...
                {name ? (
                  <Text style={styles.greeting}>{name}'s Drinks</Text>
                ) : null}
