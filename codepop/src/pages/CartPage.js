...
const CartPage = () => {
    ...
    try {
      ...
      for (let i = 0; i < currentList.length; i++) {
        const response = await fetch(
          `${BASE_URL}/backend/drinks/${currentList[i]}/`,
          {
            method: "GET",
            headers: {
  ...
  const calculatePrice = (drink) => {
    ...
    if (drink.Price == 2) {
      const syrupsCount = Array.isArray(drink.SyrupsUsed)
        ? drink.SyrupsUsed.length
        : 0;
  ...
  const renderDrinkItem = (drink) => (
    <View style={styles.drinkContainer}>
      <Text style={styles.drinkText}>
        {drink.Size} Drink: {drink.SodaUsed.join(", ")} with {drink.Ice} Ice
      </Text>
      ...
      <Text style={styles.priceText}>
        Price: ${calculatePrice(drink).toFixed(2)}
      </Text>
      ...
      <View style={styles.buttonRow}>
        ...
        <TouchableOpacity
          onPress={() => removeDrink(drink.DrinkID)}
          style={styles.button}
        >
      ...
      <View style={styles.container}>
        ...
        {Array.isArray(drinks) && drinks.length === 0 ? (
          ...
          <FlatList
            ...
            data={drinks}
            keyExtractor={(item) =>
              item.DrinkID ? item.DrinkID.toString() : Math.random().toString()
            }
