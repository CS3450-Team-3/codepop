  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        const sortedInventory = inventoryData.inventory_items.sort(
          (a, b) => a.ThresholdLevel - b.ThresholdLevel
        );
  ...
  const resetInventory = async (itemId, thresholdLevel) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "Error resetting inventory: " + errorData.detail || "Unknown error"
        );
      }
    } catch (error) {
                  ...
                  {inventory.map((item) => (
                    <View key={item.InventoryID} style={styles.inventoryItem}>
                      ...
                      <TouchableOpacity
                        style={styles.button}
                        onPress={() =>
                          resetInventory(item.InventoryID, item.ThresholdLevel)
                        }
                      >
                  ...
                  {revenue.map((rev) => (
                    <View key={rev.RevenueID} style={styles.revenueCard}>
                      <Text style={styles.revenueText}>
                        Sale Date: {formatDate(rev.SaleDate)}
                      </Text>
                      <Text style={styles.revenueText}>
                        Order ID: {rev.OrderID}
                      </Text>
                      <Text style={styles.revenueText}>
                        Amount: ${rev.TotalAmount.toFixed(2)}
                      </Text>
                    </View>
                  ))}
                </ScrollView>
